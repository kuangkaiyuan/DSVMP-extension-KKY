...
The PC runs the Windows 10 operating system. We evaluated our approach using four widely use applications: ``\texttt{md5}"~\cite{19md5}, ``\texttt{aescrypt}"~\cite{20Aescrypt}, ``\texttt{bcrypt}"~\cite{21bcrypt} and ``\texttt{gzip}"~\cite{22gzip}.
We used these applications to process a test text file. The size of the file is 26 KB.
We also used two small game applications that required user interaction to evaluate the performance of our protection system.
``\texttt{AStar}"~\cite{}, a maze pathfinding game using the algorithm of astar. ``\texttt{Tetris}"~\cite{}, a classic graphics combination game.
...
\end{table*}
\footnotetext{When ``\texttt{Tetris}" is doing the clear row operations, the patterns of the whole structure is uncertain, so the number of instructions produced by this action is also uncertain, here we use the average of multiple data.}
...
\paragraph*{Code size} For each target benchmark, we applied \DSVMP to the target function and repeated the process for 10 times.
...
For ``\texttt{AStar}" and ``\texttt{Tetris}", in order to eliminate the time impact of the interaction,
we only calculate and collect the overage run time of the target operation (pathfinding and row clear).
